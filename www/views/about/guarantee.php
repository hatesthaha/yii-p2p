<?php
use yii\helpers\Html;
$directoryAsset = yii::$app->request->baseUrl.'/../../backend/web/upload/';

$this->title = '担保机构';
?>

	
<div id="content">
	
	<div class="main gywm">
    	<div class="left" id="left">
        	<ul>
                <?php if($left[0]['status']==1){?>
                    <li class="leib1"><a href="<?= yii\helpers\Url::to(['about/company']); ?>"><?= $left[0]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[1]['status']==1){?>
                    <li class="leib2 "><a href="<?= yii\helpers\Url::to(['about/media']); ?>"><?= $left[1]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[2]['status']==1){?>
                    <li class="leib3"><a href="<?= yii\helpers\Url::to(['about/partner']); ?>"><?= $left[2]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[3]['status']==1){?>
                    <li class="leib4"><a href="<?= yii\helpers\Url::to(['about/news']); ?>"><?= $left[3]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[4]['status']==1){?>
                    <li class="leib5  hover"><a href="<?= yii\helpers\Url::to(['about/guarantee']); ?>"><?= $left[4]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[5]['status']==1){?>
                    <li class="leib6"><a href="<?= yii\helpers\Url::to(['about/join']); ?>"><?= $left[5]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[6]['status']==1){?>
                    <li class="leib7"><a href="<?= yii\helpers\Url::to(['about/contact']); ?>"><?= $left[6]['name'] ?></a></li>
                <?php
                } ?>
                <?php if($left[7]['status']==1){?>
                    <li class="leib8"><a href="<?= yii\helpers\Url::to(['help/index']); ?>"><?= $left[7]['name'] ?></a></li>
                <?php
                } ?>
            </ul>
        </div>
        <div class="right" id="right">
        	
			<h2>担保机构</h2>
            <div class="lxwm">
                <div class="hqb-dbjg">
                <?php if($infos){foreach ($infos as $K=>$V){?>
                     <dl class="dbjg-dl clearFloat">
                         <dt><img src="<?php echo $directoryAsset.$V['bannar'];?>" alt=""></dt>
                         <dd>
                            <p><?php echo mb_substr($V['intro'], 0,128,'utf8');?>
                                <span class="text-more"><?php echo mb_substr($V['intro'], 128,mb_strlen($V['intro'])-128,'utf8');?></span>              
                                <a class="js-showMore" href="javascript:;">查看详情</a>
                            </p>
                        </dd>
                     </dl>
                     <hr>
                <?php }}?>
                     
                </div>
            </div>
        </div>
        <div class="clear"></div>
    </div>
    
</div>
<script src="<?php echo Yii::$app->homeUrl;?>myAssetsLib/js/jquery-1.7.1.js"></script>
<script type="text/javascript">
    $(document).ready(function(){
        $(".dbjg-dl dd p .js-showMore").click(function(event){
            var zhanshi =  $(this).parent("p").find(".text-more");
            event.preventDefault();
            if(zhanshi.is(":hidden")){  
                $(this).html("收起详情");
                $(this).parent("p").find(".text-more").css("display","inline");

            }else{
                $(this).html("查看详情");
                $(this).parent("p").find(".text-more").css("display","none");
            };


        });
    })
</script>